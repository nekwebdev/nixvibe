...
def _payload(*, agent_id: str) -> dict[str, object]:
    return {
        "agent_id": agent_id,
        "task_scope": "phase22-foundation-acceptance",
        "status": "warning",
        "findings": [
            {
                "id": f"F-{agent_id}",
                ...
                "impact": "Phase 22 acceptance coverage",
            }
        ],
        "recommendations": [
            {
...
class TestPhase22FoundationAcceptance(unittest.TestCase):
    @classmethod
    def setUpClass(cls) -> None:
        cls.policy = load_policy(ROOT / ".agents/carl/nixvibe-domain.md")

    def test_default_flow_emits_hold_and_review(self) -> None:
        ...
        self.assertEqual(result.artifact_summary["v10_compatibility_baseline"]["compatibility_status"], "hold")
        self.assertEqual(result.artifact_summary["migration_safety_policy"]["policy_decision"], "review")

    def test_second_run_with_history_and_release_check_reaches_ready_and_allow(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            root = Path(tmp)
            _prepare_workspace(root)
            first = run_pipeline(
        ...
        self.assertEqual(second.artifact_summary["v10_compatibility_baseline"]["compatibility_status"], "ready")
        self.assertEqual(second.artifact_summary["migration_safety_policy"]["policy_decision"], "allow")

    def test_override_denied_path_blocks_foundation_policy(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            root = Path(tmp)
            _prepare_workspace(root)
            first = run_pipeline(
        ...
        self.assertEqual(second.artifact_summary["v10_compatibility_baseline"]["compatibility_status"], "blocked")
        self.assertEqual(second.artifact_summary["migration_safety_policy"]["policy_decision"], "block")
