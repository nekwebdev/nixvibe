...
def _payload(*, agent_id: str) -> dict:
    return {
        "agent_id": agent_id,
        "task_scope": "telemetry-regression-thresholds",
        "status": "warning",
        "findings": [
            {
                "id": f"F-{agent_id}",
                ...
                "impact": "Telemetry regression threshold coverage",
            }
        ],
        "recommendations": [
            {
...
class TestTelemetryRegressionThresholds(unittest.TestCase):
    @classmethod
    def setUpClass(cls) -> None:
        cls.policy = load_policy(ROOT / ".agents/carl/nixvibe-domain.md")

    def test_fast_path_passes_thresholds(self) -> None:
        ...
        report = result.artifact_summary["telemetry_regression"]
        self.assertEqual(report["contract"], "telemetry-regression/v1")
        self.assertFalse(report["regression_detected"])
        self.assertEqual(report["failed_check_ids"], ())
        self.assertEqual(report["failed_threshold_ids"], ())

    def test_slow_path_detects_threshold_regression(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            root = Path(tmp)
            (root / "flake.nix").write_text("{ }")
            result = run_pipeline(
        ...
        self.assertIn("total-duration-ms", report["failed_threshold_ids"])
        self.assertGreaterEqual(report["failed_threshold_count"], 1)
