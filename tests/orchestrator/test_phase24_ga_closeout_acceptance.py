...
def _payload(*, agent_id: str) -> dict[str, object]:
    return {
        "agent_id": agent_id,
        "task_scope": "phase24-ga-closeout-acceptance",
        "status": "warning",
        "findings": [
            {
                "id": f"F-{agent_id}",
                ...
                "impact": "Phase 24 acceptance coverage",
            }
        ],
        "recommendations": [
            {
...
class TestPhase24GACloseoutAcceptance(unittest.TestCase):
    @classmethod
    def setUpClass(cls) -> None:
        cls.policy = load_policy(ROOT / ".agents/carl/nixvibe-domain.md")

    def test_default_flow_emits_hold_launch_posture(self) -> None:
        ...
        self.assertEqual(result.artifact_summary["v10_launch_evidence_bundle"]["evidence_status"], "hold")
        self.assertEqual(result.artifact_summary["v10_launch_readiness_summary"]["readiness_status"], "hold")

    def test_second_run_with_history_and_release_check_reaches_ready_launch_posture(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            root = Path(tmp)
            _prepare_workspace(root)
            first = run_pipeline(
        ...
        self.assertEqual(second.artifact_summary["v10_launch_evidence_bundle"]["evidence_status"], "ready")
        self.assertEqual(second.artifact_summary["v10_launch_readiness_summary"]["readiness_status"], "ready")
        self.assertEqual(second.artifact_summary["v10_launch_readiness_summary"]["launch_gate"], "open")

    def test_override_denied_path_blocks_launch_closeout(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            root = Path(tmp)
            _prepare_workspace(root)
            first = run_pipeline(
        ...
        self.assertEqual(second.artifact_summary["v10_launch_evidence_bundle"]["evidence_status"], "blocked")
        self.assertEqual(second.artifact_summary["v10_launch_readiness_summary"]["readiness_status"], "blocked")
