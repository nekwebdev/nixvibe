...
def _payload(*, agent_id: str) -> dict[str, object]:
    return {
        "agent_id": agent_id,
        "task_scope": "v0.7-milestone-acceptance",
        "status": "warning",
        "findings": [
            {
                "id": f"F-{agent_id}",
...
class TestV07MilestoneAcceptance(unittest.TestCase):
    @classmethod
    def setUpClass(cls) -> None:
        cls.policy = load_policy(ROOT / ".agents/carl/nixvibe-domain.md")

    def test_apply_success_includes_phase19_to21_contract_chain(self) -> None:
        ...
        self.assertEqual(summary["release_policy_execution"]["decision"], "manual-ack")
        self.assertEqual(summary["v07_closeout_evidence"]["closeout_category"], "hold")
        self.assertEqual(summary["v10_pathway_scaffold"]["pathway_status"], "hold")

    def test_second_run_with_history_and_release_check_reaches_ready_transition(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            root = Path(tmp)
            _prepare_workspace(root)
            first = run_pipeline(
        ...
        self.assertEqual(second.artifact_summary["governance_hardening_escalation"]["escalation_level"], "none")
        self.assertEqual(second.artifact_summary["v07_closeout_evidence"]["closeout_category"], "ready")
        self.assertEqual(second.artifact_summary["v10_pathway_scaffold"]["pathway_status"], "ready")
        self.assertEqual(second.artifact_summary["v10_pathway_scaffold"]["transition_gate"], "open")

    def test_override_denied_path_blocks_closeout_and_pathway(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            root = Path(tmp)
            _prepare_workspace(root)
            first = run_pipeline(
        ...
        self.assertEqual(second.artifact_summary["governance_hardening_escalation"]["escalation_level"], "critical")
        self.assertEqual(second.artifact_summary["v07_closeout_evidence"]["closeout_category"], "blocked")
        self.assertEqual(second.artifact_summary["v10_pathway_scaffold"]["pathway_status"], "blocked")
