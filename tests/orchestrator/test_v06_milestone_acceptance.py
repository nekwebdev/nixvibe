...
def _payload(*, agent_id: str) -> dict[str, object]:
    return {
        "agent_id": agent_id,
        "task_scope": "v0.6-milestone-acceptance",
        "status": "warning",
        "findings": [
            {
                "id": f"F-{agent_id}",
...
class TestV06MilestoneAcceptance(unittest.TestCase):
    @classmethod
    def setUpClass(cls) -> None:
        cls.policy = load_policy(ROOT / ".agents/carl/nixvibe-domain.md")

    def test_apply_success_includes_phase16_to18_contract_chain(self) -> None:
        ...
        for key in (
            ...
            "benchmark_trend_history",
            "benchmark_trend_delta",
            "outcome_alert",
            "alert_policy_gate",
            "release_candidate_evidence",
            "v06_readiness_summary",
        ):
            self.assertIn(key, summary)
        self.assertEqual(summary["release_candidate_evidence"]["readiness_category"], "hold")
        self.assertEqual(summary["v06_readiness_summary"]["readiness_band"], "hold")

    def test_second_run_with_history_and_release_check_reaches_ready(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            root = Path(tmp)
            _prepare_workspace(root)
            first = run_pipeline(
        ...
        self.assertEqual(second.artifact_summary["outcome_alert"]["alert_status"], "none")
        self.assertEqual(second.artifact_summary["alert_policy_gate"]["gate_status"], "open")
        self.assertEqual(second.artifact_summary["release_candidate_evidence"]["readiness_category"], "ready")
        self.assertEqual(second.artifact_summary["release_check_command"]["status"], "passed")
        self.assertEqual(second.artifact_summary["v06_readiness_summary"]["readiness_band"], "ready")

    def test_validation_failure_path_blocks_v06_readiness(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            root = Path(tmp)
            _prepare_workspace(root)
        ...
        self.assertEqual(result.artifact_summary["outcome_alert"]["alert_status"], "critical")
        self.assertEqual(result.artifact_summary["alert_policy_gate"]["gate_status"], "blocked")
        self.assertEqual(result.artifact_summary["release_candidate_evidence"]["readiness_category"], "blocked")
        self.assertEqual(result.artifact_summary["v06_readiness_summary"]["readiness_band"], "blocked")
