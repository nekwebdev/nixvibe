...
def _payload(*, agent_id: str) -> dict[str, object]:
    return {
        "agent_id": agent_id,
        "task_scope": "phase23-operator-consolidation-acceptance",
        "status": "warning",
        "findings": [
            {
                "id": f"F-{agent_id}",
                ...
                "impact": "Phase 23 acceptance coverage",
            }
        ],
        "recommendations": [
            {
...
class TestPhase23OperatorConsolidationAcceptance(unittest.TestCase):
    @classmethod
    def setUpClass(cls) -> None:
        cls.policy = load_policy(ROOT / ".agents/carl/nixvibe-domain.md")

    def test_default_flow_emits_attention_and_review(self) -> None:
        ...
        self.assertEqual(result.artifact_summary["operator_control_plane_summary"]["control_plane_status"], "attention")
        self.assertEqual(
            result.artifact_summary["governance_workflow_consolidation"]["consolidation_status"],
            "review",
        )

    def test_second_run_with_history_and_release_check_reaches_aligned_and_consolidated(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            root = Path(tmp)
            _prepare_workspace(root)
            first = run_pipeline(
        ...
        self.assertEqual(second.artifact_summary["operator_control_plane_summary"]["control_plane_status"], "aligned")
        self.assertEqual(
            second.artifact_summary["governance_workflow_consolidation"]["consolidation_status"],
            "consolidated",
        )

    def test_override_denied_path_blocks_operator_consolidation(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            root = Path(tmp)
            _prepare_workspace(root)
            first = run_pipeline(
        ...
        self.assertEqual(second.artifact_summary["operator_control_plane_summary"]["control_plane_status"], "blocked")
        self.assertEqual(second.artifact_summary["governance_workflow_consolidation"]["consolidation_status"], "blocked")
