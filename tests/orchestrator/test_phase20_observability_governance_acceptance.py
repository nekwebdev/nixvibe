...
def _payload(*, agent_id: str) -> dict[str, object]:
    return {
        "agent_id": agent_id,
        "task_scope": "phase20-observability-governance-acceptance",
        "status": "warning",
        "findings": [
            {
                "id": f"F-{agent_id}",
                ...
                "impact": "Phase 20 acceptance coverage",
            }
        ],
        "recommendations": [
            {
...
class TestPhase20ObservabilityGovernanceAcceptance(unittest.TestCase):
    @classmethod
    def setUpClass(cls) -> None:
        cls.policy = load_policy(ROOT / ".agents/carl/nixvibe-domain.md")

    def test_default_flow_emits_attention_and_escalation(self) -> None:
        ...
        self.assertEqual(
            result.artifact_summary["operator_observability_digest"]["observability_band"], "attention"
        )
        self.assertEqual(
            result.artifact_summary["governance_hardening_escalation"]["escalation_level"], "escalate"
        )

    def test_second_run_with_history_and_release_check_reaches_healthy_stable_posture(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            root = Path(tmp)
            _prepare_workspace(root)
            first = run_pipeline(
        ...
        self.assertEqual(second.artifact_summary["operator_observability_digest"]["observability_band"], "healthy")
        self.assertEqual(second.artifact_summary["governance_hardening_escalation"]["escalation_level"], "none")
        self.assertEqual(second.artifact_summary["governance_hardening_escalation"]["governance_posture"], "stable")

    def test_override_denied_path_blocks_governance_posture(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            root = Path(tmp)
            _prepare_workspace(root)
            first = run_pipeline(
        ...
        self.assertEqual(second.artifact_summary["operator_observability_digest"]["observability_band"], "critical")
        self.assertEqual(
            second.artifact_summary["governance_hardening_escalation"]["escalation_level"], "critical"
        )
        self.assertEqual(
            second.artifact_summary["governance_hardening_escalation"]["governance_posture"], "blocked"
        )
